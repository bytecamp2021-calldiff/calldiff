...
func TestWalk(t *testing.T) {
	dir := `/Users/bytedance/Desktop/code/Dragonfly/`
	cfg := &packages.Config{
		Mode: packages.LoadSyntax | packages.NeedDeps,
		Dir:  dir,
	}
	var initials []*packages.Package
	filepath.Walk(dir, func(path string, info fs.FileInfo, err error) error {
		if strings.HasSuffix(path, ".go") {
			fmt.Println(path)
		}
		fmt.Println("!!!", path)
		initial, err := packages.Load(cfg, path[len(dir):])
	...
	prog, pkgs := ssautil.Packages(initials, 0)
	prog.Build()
	fmt.Println(pkgs)
}
...
func TestCallgraph(t *testing.T) {
	...
	for _, test := range []struct {
		...
		if err := doCallgraph("testdata", gopath, test.algo, format, test.tests, []string{"pkg"}); err != nil {
			t.Error(err)
			continue
		}
